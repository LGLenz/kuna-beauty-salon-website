...
**Kuna's Beauty Salon** (`https://kuna-beauty.bridgeaxis-consulting.de` — also
...
| custom-domain  | https://kuna-beauty.bridgeaxis-consulting.de                               | no (warn-only)  |

Both targets are **warn-only** today (2026-05-22) because:

1. DNS for `kuna-beauty.bridgeaxis-consulting.de` still resolves to the legacy ELB
...
set to `kuna-beauty.bridgeaxis-consulting.de` and tick "Enforce HTTPS" once the
...
| `kuna-beauty.bridgeaxis-consulting.de` shows cert error    | CNAME not yet added in the elbconsultingtech.com zone, or TLS still being issued. | `site-health.yml` → `http-check` (custom-domain target, warn-only) + `dns-check` job summary. |
...
3. **GitHub Pages custom domain** (Settings → Pages) — confirm the value
   `kuna-beauty.bridgeaxis-consulting.de`; do NOT tick "Enforce HTTPS" until the
   certificate has been provisioned.
