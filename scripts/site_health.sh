...
http_code=$(curl -sS -L --max-time 15 \
  -A "Kuna-SiteHealth/1.0 (+https://github.com/${GITHUB_REPOSITORY:-LGLenz/kuna-beauty-salon-website})" \
...
elif ! echo "$title" | grep -qiE "kuna|kush"; then
  fail "Title '$title' does not mention 'Kuna' or 'KUSH'"
fi
